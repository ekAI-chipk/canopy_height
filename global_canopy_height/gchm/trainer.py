...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(epoch_start, self.args.nb_epoch):
            ...
            if epoch % 10 == 0:
                ...
                val_dict, val_metrics = None, None
            # val_dict, val_metrics = self.validate(dl_val) # validate with num_iterations = len(dl_val)
            ...
            # tensorboard
            if val_dict is not None and val_metrics is not None:
                metric_string = 'VAL:   '
                for metric in metric_keys:
                    # tensorboard logs
            ...
            if not self.args.debug:
                if np.isnan(training_metrics[self.args.loss_key]) or (val_metrics is not None and np.isnan(val_metrics[self.args.loss_key])):
                    print(training_metrics[self.args.loss_key])
                    
                    if val_metrics is not None:
                        print(val_metrics[self.args.loss_key])
            ...
            torch.save({
                ...
                'val_loss': val_metrics[self.args.loss_key] if val_metrics is not None else None,
            ...
            if epoch % 50 == 0:
                ...
                if not os.path.exists(folder_path):
                    os.makedirs(folder_path)  # makedirs creates parent directories if needed
                    print(f"Folder created: {folder_path}")
                
                checkpoint_path = Path(self.args.out_dir) / f'checkpoints/checkpoint_{epoch}.pt'
                torch.save({
                    'epoch': epoch,
                    'model_state_dict': self.model.state_dict(),
                    'optimizer_state_dict': self.optimizer.state_dict(),
    ...
    def optimize_epoch(self, dl_train, epoch):
        ...
        with profiler_context_manager as profiler:
            ...
            if not self.args.iterations_per_epoch or self.args.iterations_per_epoch == 0:
    ...
    def validate(self, dl_val, num_iterations=None, compute_loss=True):
        ...
        # print("unique: val_dict['targets']", torch.unique(torch.tensor(val_dict['targets'])))
        # print("val_dict['targets'].dtype", torch.tensor(val_dict['targets']).dtype)
